<div class="flex items-start gap-10 max-w-4xl mx-auto mt-10">
  ...
  <div class="flex-1 bg-gray-900 text-gray-100 p-6 rounded-lg border border-gray-700">
    <h2 class="text-2xl font-semibold mb-4">{{__('ui.categories_categories_block_title')}} ({{count($data)}})</h2>
            ...
            @foreach($data as $cat)
                ...
                <div class="category flex items-center justify-between bg-gray-800 hover:bg-gray-700 p-3 rounded" data-category-id="{{ $cat->id }}">
                    ...
                    <div class="flex gap-2">
                        <a href='/categories/edit/{{$cat->id}}' class="px-3 py-1 border border-[var(--accent-2)] text-[var(--accent-3)] rounded hover:bg-[var(--accent-2)]/20 transition text-sm opacity-50 hover:opacity-100">{{__('ui.edit_btn')}}</a>

                        <form action="{{ route('category.delete', $cat['id']) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button onclick="return confirm('Are you sure you want to delete it?\nThis action cannot be undone.')" type="submit" class="px-3 py-1 h-[30px] bg-red-600 text-white rounded hover:bg-red-700 transition text-sm opacity-40 hover:opacity-100">{{__('ui.delete_btn')}}</button>
                        </form>
                    </div>
                </div>
            @endforeach
        @else
            <div class="text-[gray]">{{__('ui.categories_nothing_msg')}}</div>
        @endif
    </div>
  </div>
  ...
  <div class="flex-1 bg-gray-900 text-gray-100 p-6 rounded-lg border border-gray-700">
    <h2 class="text-2xl font-semibold mb-4">
        {{ $flag === 'edit' ? __('ui.categories_form_title_edit') : __('ui.categories_form_title_add') }}
    </h2>
    <form action="{{ $flag === 'edit' ? route('category.edit', $entry['id']) : route('category.add') }}" method="POST" class="flex flex-col gap-3">
        ...
        @if ($flag === 'edit')
            @method('PUT')
        @endif
      <input name="name" type="text" placeholder="{{__('ui.categories_form_placeholder')}}" autofocus="true"
      ...
      <button type="submit" class="mt-2 bg-[var(--accent-2)] text-black py-2 rounded hover:bg-[var(--accent-4)] transition">
        {{ $flag === 'edit' ? __('ui.edit_btn') : __('ui.add_btn') }}
      </button>
    </form>
    @error('name')
